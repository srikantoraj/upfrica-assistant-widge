<script>
/* Upfrica Chat Widget (shadow-DOM, Tailwind-proof)
   - Self-styled; no Tailwind usage so external Tailwind won't affect it
   - Preserves your endpoint contract: { output, sessionId }
   - Supports JSON directives:
       { products:[...], seeAllLink, cta }  -> product cards
       { ui:"form", id, title, fields:[...]} -> form bubble
       { message:"..." } or plain string     -> text bubble
*/
(function () {
  ...
  // ——— Utilities ———
  const svg = {
    bot:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
    ...
    chat:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="currentColor" aria-hidden="true"><path d="M20 2H4a2 2 0 00-2 2v16l4-4h14a2 2 0 002-2V4a2 2 0 00-2-2z"/></svg>`,
    close:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M6.225 4.811L4.811 6.225 10.586 12l-5.775 5.775 1.414 1.414L12 13.414l5.775 5.775 1.414-1.414L13.414 12l5.775-5.775-1.414-1.414L12 10.586 6.225 4.811z"/></svg>`,
    minus:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><rect x="4" y="11" width="16" height="2" rx="1"/></svg>`,
    send:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true" style="transform:rotate(-45deg)"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>`
  };
  const css = `
:host{all:initial}
...
:root{
  ...
  --bg:#fff;--fg:#111827;--muted:#6b7280;--panel:#e5e7eb;--chip:#f3f4f6;--bubble:#f3f4f6;--shadow:0 25px 50px -12px rgba(0,0,0,.25);
  --ring:rgba(79,70,229,.35);--scroll:#c7c7c7
}
...
.${NS}-launcher:hover{background:var(--indigo-700)}
.${NS}-launcher:focus-visible{outline:2px solid var(--ring);outline-offset:2px}
...
.${NS}-iconbtn:hover{background:rgba(0,0,0,.05)}
@media (prefers-color-scheme:dark){.${NS}-iconbtn:hover{background:rgba(255,255,255,.06)}}
...
.${NS}-scroll::-webkit-scrollbar-thumb{background:var(--scroll);border-radius:9999px}

.${NS}-row{display:flex;gap:.5rem}
.${NS}-row-user{justify-content:flex-end;align-items:flex-end}
.${NS}-row-ai{align-items:flex-start}
...
.${NS}-send:hover{background:var(--indigo-700)}
.${NS}-send[disabled]{opacity:.5;cursor:not-allowed}
...
.${NS}-card>a:hover{background:rgba(0,0,0,.04)}
@media (prefers-color-scheme:dark){.${NS}-card>a:hover{background:rgba(255,255,255,.06)}}
...
`;
  ...
  function supportsSmooth(){return !window.matchMedia?.('(prefers-reduced-motion: reduce)').matches}
  function stripFence(s){if(!s)return s;const t=String(s).trim();return t.startsWith("```")?t.replace(/^```[a-z0-9]*\n?/i,"").replace(/\n```$/,""):s}
  ...
  function productCardsHTML({products=[],seeAllLink='',cta=''}) {
    ...
    const seeAll = seeAllLink ? `<a class="${NS}-title-sm" style="display:inline-flex;gap:.25rem;align-items:center;color:var(--indigo-600);text-decoration:none" href="${safeUrl(seeAllLink)}" target="_blank" rel="noopener nofollow noreferrer">See all results
      <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M13 5l7 7-7 7v-4H4v-6h9V5z"/></svg></a>` : '';
  ...
  function tryParseOutput(outputStr){
    const raw = stripFence(outputStr);
    try{
      const j = JSON.parse(raw);
      if (Array.isArray(j.products)) return {kind:'products', data:{products:j.products, seeAllLink:j.seeAllLink||'', cta:j.cta||''}};
      if (j && j.ui==='form' && Array.isArray(j.fields)) return {kind:'form', data:j};
  ...
  // ——— DOM builders ———
  function rowAI(html){ const r=document.createElement('div'); r.className=`${NS}-row ${NS}-row-ai`;
    r.innerHTML=`<div class="${NS}-chip-bot" aria-hidden="true">${svg.bot}</div><div class="${NS}-bubble ${NS}-bubble-ai">${html}</div>`; return r; }
  function rowUser(text){ const r=document.createElement('div'); r.className=`${NS}-row ${NS}-row-user`;
    r.innerHTML=`<div class="${NS}-bubble ${NS}-bubble-user">${escapeHtml(text)}</div><div class="${NS}-chip-you">You</div>`; return r; }
  function rowTyping(){ const r=document.createElement('div'); r.className=`${NS}-row ${NS}-row-ai`; r.dataset.typing='true';
    r.innerHTML=`<div class="${NS}-chip-bot" aria-hidden="true">${svg.bot}</div><div class="${NS}-bubble ${NS}-bubble-ai">
      <span class="${NS}-dot ${NS}-dot-1"></span><span class="${NS}-dot ${NS}-dot-2" style="margin-left:.15rem"></span><span class="${NS}-dot ${NS}-dot-3" style="margin-left:.15rem"></span></div>`; return r; }
  ...
  function formBubble(spec){
    ...
    const inputs = fields.map(f=>{
      ...
      const min = f.min!==undefined?` min="${f.min}"`:''; const max = f.max!==undefined?` max="${f.max}"`:''; const step = f.step!==undefined?` step="${f.step}"`:'';
    ...
    wrap.innerHTML = `<div class="${NS}-chip-bot" aria-hidden="true">${svg.bot}</div>
      <div class="${NS}-bubble ${NS}-bubble-ai" style="padding:.75rem">
        <div style="font-weight:600;margin:0 0 .25rem">${escapeHtml(title)}</div>
        ${msg}
        <form id="${formId}" data-form-id="${escapeHtml(id)}" style="display:block">${inputs}
          <div style="padding-top:.5rem"><button type="submit" class="${NS}-send">${svg.send}<span>${escapeHtml(submitLabel)}</span></button></div>
        </form>
      </div>`;
    // submit handler
    setTimeout(()=>{
      ...
      form.addEventListener('submit', async (e)=>{
        ...
        const data={}; Array.from(form.elements).forEach(el=>{ if(!el.name) return; if(el.type==='checkbox') data[el.name]=el.checked; else data[el.name]=el.value; });
        const typing = rowTyping(); messages.appendChild(typing); scrollToBottom(messages,true);
        let output; try{
          const resp = await sendToAssistant({ message:"[FORM_SUBMISSION]", event:"form_submitted", id: form.dataset.formId, data });
          output = resp;
  ...
  function scrollToBottom(el,smooth=true){ el.scrollTo({ top: el.scrollHeight+9999, behavior: (smooth && supportsSmooth())?'smooth':'auto' }); }
  ...
  function renderOutput(outputStr){
    const parsed = tryParseOutput(outputStr);
    if (parsed.kind==='products'){
      messages.appendChild(rowAI(productCardsHTML(parsed.data))); scrollToBottom(messages,true); return;
    }
    if (parsed.kind==='form'){
      messages.appendChild(formBubble(parsed.data)); scrollToBottom(messages,true); return;
    }
    messages.appendChild(rowAI(escapeHtml(parsed.data).replace(/\n/g,"<br>"))); scrollToBottom(messages,true);
  }

  // ——— Mount widget ———
  let sessionId = localStorage.getItem(CFG.sessionKey) || null;

  // Hide any pre-existing demo markup (your page has #chat-launcher/#chat-panel)
  const demoLauncher = document.getElementById('chat-launcher');
  const demoPanel = document.getElementById('chat-panel');
  ...
  // Create host + shadow
  const host = document.createElement('div'); host.id = `${NS}-container`;
  ...
  launcher.className = `${NS}-launcher ease`; launcher.type='button'; launcher.setAttribute('aria-label','Open chat'); launcher.setAttribute('aria-expanded','false');
  launcher.innerHTML = svg.chat;
  shadow.appendChild(launcher);

  // panel
  const panel = document.createElement('section');
  panel.className = `${NS}-wrap ${NS}-hidden`;
  panel.setAttribute('role','dialog'); panel.setAttribute('aria-modal','true'); panel.setAttribute('aria-label','Chat panel'); panel.setAttribute('aria-live','polite');

  const header = document.createElement('header');
  header.className = `${NS}-header`;
  header.innerHTML = `<div class="${NS}-title"><div class="${NS}-brand" aria-hidden="true">${svg.bot}</div><div id="${NS}-heading">${CFG.brandName}</div></div>
  ...
  messages.className = `${NS}-messages ${NS}-scroll`;
  messages.innerHTML = `<div class="${NS}-row ${NS}-row-ai"><div class="${NS}-chip-bot" aria-hidden="true">${svg.bot}</div>
    <div class="${NS}-bubble ${NS}-bubble-ai">Hi! I’m <strong>${escapeHtml(CFG.brandName)}</strong>. Ask me anything — I’ll reply here. 👋</div></div>`;
  panel.appendChild(messages);

  const form = document.createElement('form'); form.className = `${NS}-composer`;
  form.innerHTML = `<div class="${NS}-composerbox">
      <textarea rows="1" class="${NS}-textarea" placeholder="Type your message…" aria-label="Type your message"></textarea>
      <button type="submit" class="${NS}-send" disabled aria-disabled="true">${svg.send}<span>Send</span></button></div>`;
  ...
  // handlers
  const minBtn = header.querySelectorAll('button')[0];
  const closeBtn = header.querySelectorAll('button')[1];
  const textarea = form.querySelector('textarea');
  const sendBtn = form.querySelector('button[type="submit"]');

  function openPanel(){
    panel.classList.remove(`${NS}-hidden`); launcher.classList.add(`${NS}-hidden`); launcher.setAttribute('aria-expanded','true');
    panel.addEventListener('keydown', trapFocus);
    setTimeout(()=>textarea.focus(), 50); scrollToBottom(messages,true);
  }
  function closePanel(){
    panel.classList.add(`${NS}-hidden`); launcher.classList.remove(`${NS}-hidden`); launcher.setAttribute('aria-expanded','false');
    panel.removeEventListener('keydown', trapFocus); launcher.focus();
  }
