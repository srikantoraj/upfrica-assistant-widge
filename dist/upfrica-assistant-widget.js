...
<head>
  <meta charset="utf-8" />
  <title>Upfrica assistant – Isolated (no borders, no gray, no rings)</title>
...
<style>
  :root{
    ...
    --bg:#ffffff;--fg:#111827;--muted:#6b7280;--panel:#e5e7eb;--shadow:0 25px 50px -12px rgba(0,0,0,.25)
  }
  @media (prefers-color-scheme:dark){
    :root{--bg:#0b0b10;--fg:#e5e7eb;--muted:#9ca3af;--panel:#262936}
  }
  ...
  *{box-sizing:border-box;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif}
  ...
  .launcher{
    ...
    outline:0 !important; box-shadow:none !important; -webkit-tap-highlight-color:transparent;
  }
  .launcher:hover{ background:var(--indigo-700); }

  /* Panel (NO BORDER, white bg only) */
  .panel{
    position:absolute; right:0; bottom:0; width:420px; max-width:92vw; height:560px; max-height:88vh;
    background:var(--bg); color:var(--fg); border:none; border-radius:16px;
  ...
  /* Bubbles: assistant = plain text (NO GRAY BG), user = blue bubble */
  .bubble{max-width:85%;font-size:.95rem;line-height:1.5}
  .bubble-ai{padding:0; background:transparent}
  .bubble-user{padding:.5rem .65rem; background:var(--indigo-600); color:#fff; border-radius:1rem; border-top-right-radius:.25rem}

  /* Composer (NO gray bg, NO borders/rings) */
  ...
  .textarea{
    flex:1; min-height:1.75rem; max-height:10rem; background:transparent; border:none; resize:none;
    outline:0 !important; box-shadow:none !important; -webkit-appearance:none; -webkit-tap-highlight-color:transparent;
    font-size:.95rem; color:var(--fg); padding:.25rem .2rem;
  }
  .textarea::placeholder{color:var(--muted); opacity:.7}
  .textarea:focus{outline:0 !important; box-shadow:none !important; border:none !important}
  ...
  /* Typing */
  .dot{width:.375rem;height:.375rem;background:#6b7280;border-radius:9999px;display:inline-block}
  .dot-1{animation:blink 1.2s infinite .0s}.dot-2{animation:blink 1.2s infinite .2s}.dot-3{animation:blink 1.2s infinite .4s}

  /* Product list (kept minimal) */
...
<body>
  <div class="wrap" aria-live="polite">
    ...
    <section class="panel hidden" id="panel" role="dialog" aria-modal="true" aria-label="Chat panel">
      <header class="header">
        ...
        <div>
          <button class="iconbtn" id="btn-min" title="Minimize" aria-label="Minimize">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"><rect x="4" y="7" width="8" height="2" rx="1"/></svg>
          </button>
          <button class="iconbtn" id="btn-close" title="Close" aria-label="Close">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"><path d="M6.225 4.811L4.811 6.225 10.586 12l-5.775 5.775 1.414 1.414L12 13.414l5.775 5.775 1.414-1.414L13.414 12l5.775-5.775-1.414-1.414L12 10.586 6.225 4.811z"/></svg>
          </button>
        </div>
      </header>
  ...
  (function(){
    ...
    // parent sizing
    function postResize(w,h,rounded){ window.parent.postMessage({__upfrica__:true,type:"resize",w,h,rounded},"*"); }
    ...
    // input behavior (no borders/rings)
    ...
    function productCardsHTML({products=[],seeAllLink='',cta=''}) {
      const items = products.map(p=>{
        const title=esc(p.title||'Untitled'), price=esc(p.price||''), link=safeUrl(p.detailsUrl||'#'), img=safeUrl(p.image||'');
    ...
    function render(out){
      ...
      if (parsed.kind==='products'){ messages.appendChild(rowAI(productCardsHTML(parsed.data))); scrollToBottom(); return; }
    ...
    composer.addEventListener('submit', async (e)=>{
      ...
      if (text.startsWith('/')) {
        ...
        if (text.toLowerCase().startsWith('/products')) { messages.appendChild(rowAI(productCardsHTML({ products: sampleProducts() }))); scrollToBottom(); return; }
        if (text.toLowerCase().startsWith('/form')) { messages.appendChild(formBubble(sampleFormSpec())); scrollToBottom(); return; }
        messages.appendChild(rowAI('Unknown command. Try <code>/products</code> or <code>/form</code>.')); scrollToBottom(); return;
      }
      ...
      catch { out = "Sorry, I couldn't reach the server. Please try again."; }
